...
def Holland_Green_Powell_2018_ds633_MeltEOS(T,P,sio2,al2o3,mgo,feo,cao,na2o,k2o,tio2,mno,p2o5,cr2o3,h2o,method = 'array'):
	...
	if method == 'array':
		...
		with open(os.devnull, 'w') as fnull, contextlib.redirect_stdout(fnull), contextlib.redirect_stderr(fnull):
			comp = np.array([sio2,al2o3,mgo,feo,cao,
			na2o,k2o,tio2,mno,p2o5,cr2o3,h2o]) * 1e-2 #converting to fraction, has to sum up to 1 instead of 100
			
			melt = melt_class(comp)
			
			melt.set_state(P*1e9,T) #Pa and K --> GPa to Pa
